...
//        loadLoggedUserInfo();
    ...
    private void generatePDF(String filename, String titleText, String[][] tableData) {
        try {
            ...
            doc.add(new Paragraph("Position:     Inventory Officer"));
            doc.add(new Paragraph("\n"));
...
//            doc.add(sig);
    ...
    private void exportBorrowersPDF() {
        try {
            ...
            List<Borrower> list = dao.getAllBorrowers();
...
// -----------------------
    ...
    private void exportBarcodesPDF(List<Item> items) {
        try {
            ...
            int columns = 2;   // 2 labels per row
            PdfPTable table = new PdfPTable(columns);
            table.setWidthPercentage(100);
            table.setWidths(new float[]{1, 1}); // equal column width
            ...
            for (Item item : items) {
                ...
                name.setSpacingAfter(6); // spacing under item name
                ...
                bean.setModuleWidth(0.30);    // thickness
                bean.setBarHeight(12);        // height
                ...
                barcodeImg.scaleAbsolute(160, 32);  // size of barcode
                barcodeImg.setAlignment(Image.ALIGN_CENTER);
                barcodeImg.setSpacingAfter(4);      // spacing under barcode
                cell.addElement(barcodeImg);

                // -----------------------------
                // BARCODE TEXT (ONLY ONCE)
                // -----------------------------
                Paragraph codeLabel = new Paragraph(
                        item.getBarcode(),
                        new Font(Font.HELVETICA, 10) // requested size
                );
                codeLabel.setAlignment(Element.ALIGN_CENTER);
                codeLabel.setSpacingAfter(8); // spacing under text
//                cell.addElement(codeLabel);
            ...
            showBarcodeSuccessPopup("Barcodes exported successfully!");
